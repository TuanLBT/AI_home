...
def print_brain_event(event: dict):
    if event["type"] == "BRAIN_DECISION":
        proposal = event.get("proposal") or {}

        print(
            f'>>> BRAIN {event["entity_id"]}: '
            f'proposal={proposal.get("action")} '
            f'final={event.get("final_action")} '
            f'approved={event.get("approved")} '
            f'reason={event.get("reason")}'
        )
    else:
        print(
            f'>>> BRAIN ERROR {event["entity_id"]}: '
            f'{event.get("error")}'
        )
...
def build_brain_snapshot(
    ...
    *,
    ...
    person_state = {
        "id": entity_id,
        "present": getattr(person, "present", False) if person is not None else False,
        "posture": getattr(person, "posture", None) if person is not None else None,
        "motion": getattr(person, "fused_motion", None) if person is not None else None,
        "horizontal_state": getattr(person, "horizontal_state", None) if person is not None else None,
        "depth_state": getattr(person, "depth_state", None) if person is not None else None,
        "gestures": sorted(getattr(person, "active_gestures", set())) if person is not None else [],
    }

    return {
        "robot": {
            "state": behavior_engine.get_state(entity_id),
        },
        "person": person_state,
        "speech": speech,
        "trigger": dict(trigger or {}),
        "feedback": world.get_action_feedback(entity_id),
    }
...
def main():
    ...
    brain = BrainWorker(
        host="http://192.168.128.120:11434",
        model="qwen3:8b",
        timeout=30.0,
        max_pending=1,
    )
                ...
                if event["type"] == "ASR_TEXT":
                    ...
                    for voice_event in voice_events:
                        if voice_event["type"] == "VOICE_UTTERANCE":
                            ...
                            entity_id = voice_event.get("entity_id")

                            if entity_id is not None:
                                brain_accepted = brain.submit(
                                    entity_id=entity_id,
                                    world_state=build_brain_snapshot(
                                        world,
                                        behavior_engine,
                                        entity_id,
                                        speech=voice_event.get("text"),
                                        trigger=voice_event,
                                    ),
                                    timestamp=now,
                                )

                                if not brain_accepted:
                                    print(
                                        ">>> BRAIN: queue full, shadow job dropped"
                                    )
